...
#コマンド作成
idx_command <- readLines('data/sql/figS5_idx.sql')
idx_command <- idx_command[!grepl("^-",idx_command)]
idx_command <-  paste(idx_command,collapse = "")

df_command <- readLines('data/sql/figS5_2_df.sql')
...
for (organism in organism_list){
    ...
    write.tree(
        tree,
        paste0("output/figS4/nwk/wilout_ncRNA_",organism,".nwk")
    )
    ...
    #保存
    ggsave(
        paste0("output/figS4/tree/wilout_ncRNA_",organism,".pdf"),
